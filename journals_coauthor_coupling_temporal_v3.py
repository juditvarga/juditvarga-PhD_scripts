...
''' MANUALLY LOOP FROM HERE FOR EVERY YEAR TO PRODUCE TEMPORAL FILES '''

# could re-write all below into a big loop with functions, but had no time
...
print(journals_cited_authors_df.columns) # Index(['SO', 'AU_NO_cited', 'SO_ran_id'], dtype='object')

from bib_couple_waltmann_etal import bib_couple_function_waltmann_etal

normalised_edge_2 = bib_couple_function_waltmann_etal(journals_cited_authors_df)

#%%
print(normalised_edge_2.columns)
journal_auth_coupling_cos_sim_df = normalised_edge_2[['weight', 'paper_1', 'paper_2']]
journal_auth_coupling_cos_sim_df.columns =  ['journal_auth_cosine_similarity', 'journal_1', 'journal_2']
print(journal_auth_coupling_cos_sim_df.columns) # Index(['journal_auth_cosine_similarity', 'journal_1', 'journal_2'], dtype='object')
print(journal_auth_coupling_cos_sim_df.head(5))
#%% *CLASSIFY EDGES ACCORDING TO DISCIPLINES*
#%%
#%% 9. create citing papers df with SC s joined
...
directory = 'P:/thesis/thesis_final_data/produced_data/NEESnorm_journals_bib_coupling/temporal/{}/'.format(catf)
...
edges_classified_df.to_csv(directory + 'edges_journals_aubibcouple_{}_{}.csv'.format(catf, year), index = False)
...
print(inter_edges_2.shape) 
#inter_edges.to_csv(directory + 'interedges_journals_aubibcouple_{}_{}_{}.csv'.format(cos_sim_for_filename, catf, year), 
#                   index = False)

inter_edges.to_csv(directory + 'interedges_journals_aubibcouple_{}_{}.csv'.format(catf, year), 
                   index = False)
